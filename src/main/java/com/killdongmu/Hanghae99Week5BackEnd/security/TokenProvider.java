...
public class TokenProvider {
    ...
    public TokenProvider(@Value("#{environment['secret.key']}") String secretKey, MemberRepository memberRepository) {
        ...
        // secretKey 복호화
        // Keys 메소드를 사용해 암호화 예정이므로 byte[] 사용
        // io.jsonWebtoken.io.Decoders
        byte[] keyBites = Decoders.BASE64.decode(secretKey);
        // Keys : SecretKeys 및 KeyPairs를 안전하게 생성하기 위한 유틸 클래스
    ...
    public TokenDto generateTokenDto(Authentication authentication) {
        ...
        // 인증 정보에서 권한을 꺼내와서 스트림으로 만든 후, (GrantedAuthority.s)-> s.getAuthority() 로 변형
        // authorities 문자열에 , 로 나눠서 담는다
